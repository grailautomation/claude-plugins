...
- Route BigQuery, Guru, Context7, and Cloudflare through their maintained CLIs
  rather than default MCP. Pilot Hex, Linear, Microsoft 365, Notion,
  ServiceNow, and Intercom CLI workflows, but keep their MCP configs until
  auth, JSON output, write safeguards, and workflow coverage are validated.
  For Microsoft 365, validate against real cloud services rather than a local
  desktop Office install. For ServiceNow, `snc` means the official ServiceNow
  Store/GitHub client; npm `snc` is unrelated and `@servicenow/cli` /
  `now-cli` is an app-development surface, not the generic ITSM record
  replacement.
